namespace ServeIt.Services.Data.Menus
{
    ...
    public class MenusService : IMenusService
    {
        ...
        public async Task AddDish(AddDishInputModel model, string menuId)
        {
            ...
            var foodStyle = this.foodStylesRepository.All().FirstOrDefault(x => x.Name == model.FoodStyle.ToLower());

            var category = this.categoriesRepository.All().FirstOrDefault(x => x.Name == model.CategoryName.ToLower());
        ...
        {
            var menuId = this.restaurantRepository.All().Where(x => x.Id == restaurantId).Select(x => x.MenuId).FirstOrDefault();
        ...
        public async Task<ICollection<string>> TakeAllCategoriesByMenu(string menuId)
        {
            var categories = this.dishesRepostory.All().Where(x => x.MenuId == menuId)
                 .Select(x => this.helperService.MakeFirstLetterCapital(x.Category.Name)).Distinct().ToList();
        ...
        public async Task<ICollection<DishViewModel>> TakeAllDishes(string menuId)
        {
            var menu = this.dishesRepostory.All().Where(x => x.MenuId == menuId)
                ...
                {
                    Name = this.helperService.MakeFirstLetterCapital(x.Name),
                    Price = x.Price,
                    Id = x.Id,
                    CategoryName = this.helperService.MakeFirstLetterCapital(x.Category.Name),
